;
; Prism setup, the picture: a video loop with the UI composited over it.
;
; NSIS has no video control, so setup plays the loop itself. Each tick it decodes
; one JPEG through GDI+, alpha-blends the screen's overlay on top, writes the two
; things that change at runtime (the install path, the progress fill) with plain
; GDI, and asks one static control to repaint. Everything lands in a single DIB,
; so nothing flickers and there are no sibling controls to fight over z-order.
;
; It also means setup has no clickable controls at all. The same tick asks
; Windows where the pointer is and whether its button just went down, which is
; how hover, clicks and dragging a frameless window all work here.
;

!include "installer\over.nsh"

!define FRAMES 379         ; the whole clip, 16 seconds at 24 fps
!define TICK   42
...
Function PrismCanvas
  ...
  ; BITMAPINFOHEADER, top-down (negative height) so it matches how images decode.
  ; biPlanes and biBitCount are WORDs: writing them as ints shifts every field
  ; after them and CreateDIBSection quietly returns nothing.
  ; Two buffers, and the front one is never selected into a DC by us. We draw
  ; into the back buffer, then copy its pixels straight into the front buffer's
  ; memory. That avoids two separate causes of the white flash: a bitmap we hold
  ; selected cannot be painted by the static, and STM_SETIMAGE invalidates the
  ; control *with erase*, so the dialog background is painted before every frame.
  IntOp $1 0 - $CanvasH
  ...
  ; The white flash is the dialog erasing its own face colour underneath us: a
  ; saved frame of it was the whole client area in #F0F0F0. WS_CLIPCHILDREN
  ; forbids a parent from painting where a child already covers, which here is
  ; everywhere.
  System::Call 'user32::GetWindowLong(p $0, i -16) i .r6'
  IntOp $6 $6 | 0x02000000
  System::Call 'user32::SetWindowLong(p $0, i -16, i $6)'

  ; Plain SS_BITMAP rather than SS_REALSIZECONTROL: the bitmap is already the
  ; control's exact size, so it is blitted instead of erased-then-stretched, and
  ; stretching a million pixels takes long enough to be caught half done.
  ...
  ; The flash is the static erasing ITSELF. A bitmap static asks its parent for
  ; a brush through WM_CTLCOLORSTATIC, fills its whole client area with it, and
  ; only then draws the bitmap: that fill is the dialog face colour, which is
  ; the #F0F0F0 frame we caught. Clipping the parent cannot help, because the
  ; child is the one painting. A hollow brush removes the fill entirely, so the
  ; previous frame stays on screen until the new one lands on top of it.
  ...
  StrCpy $Hot ""
  StrCpy $Leaving 0
  ; assume the button may still be down from the click that brought us here, so
  ; the next page needs a real release before it accepts anything
  StrCpy $WasDown 1
FunctionEnd

Function PrismCanvasFree
  ${If} $BoxOn <> 0
...
Function PrismDraw
  ...
  ; 2. this screen's overlay, in its current hover state, alpha and all
  ${If} $OverImg <> 0
  ...
  ; 3. the two things a bitmap cannot know: where it is going, and how far along
  ${If} $Screen = 1
    !insertmacro OAT WHERE_PATH
    System::Call 'gdi32::SetBkMode(p $MemDC, i ${TRANSPARENT})'
    System::Call 'gdi32::SetTextColor(p $MemDC, i 0xE8E2E0)'
  ...
  ; Finish the frame, hand the DC back its own bitmap, then copy the pixels into
  ; the bitmap the static holds and repaint without erasing. No STM_SETIMAGE, so
  ; nothing paints the background between frames.
...
; Load the overlay named by $1 (for example "welcome" or "where-hot-next"), but
; only when it is not the one already loaded.
...
; ---- input -------------------------------------------------------------------
; No control is clickable, because there are no controls: the same tick that
; draws the picture asks where the pointer is and whether its button just fell.
Function PrismInput
  System::Call '*(i,i) p.s'
  Pop $9
  ...
  ${If} $Hot == ""
    ${If} $Screen = 0
      ...
      ${EndIf}
    ${ElseIf} $Screen = 1
      !insertmacro HITS WHERE_NEXT $0
      ${If} $0 = 1
        StrCpy $Hot "next"
      ${Else}
        !insertmacro HITS WHERE_BACK $0
        ${If} $0 = 1
          StrCpy $Hot "back"
        ${Else}
          !insertmacro HITS WHERE_BROWSE $0
          ${If} $0 = 1
            StrCpy $Hot "browse"
          ${EndIf}
        ${EndIf}
  ...
  ; A click is an edge. The low bit of GetAsyncKeyState means "pressed since I
  ; last asked", which catches a click that began and ended between two ticks;
  ; the high bit is the button's state right now. Only our own window's clicks
  ; count, or a click in another app would press whatever we are hovering.
...
; Post, never send. SendMessage would run the whole page change inside this
; callback, and NSIS would then destroy the dialog whose WM_TIMER we are
; standing in: CoreMessaging takes that badly (0xC00001AD).
!macro POST_CMD ID
  StrCpy $Leaving 1
...
Function PrismClick
  ...
  ; runs its own loop inside this call, so the picture holds still until you let
  ; go, which is the one place that is fine.
  IntOp $8 38 * $Dpi
  IntOp $8 $8 / 96
  ${If} $7 < $8
