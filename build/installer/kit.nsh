;
; Prism setup, part one: the window.
;
; There is no wizard here. NSIS's frame, header, hairline and three buttons are
; all removed, the window is sized to the picture, and everything you see after
; that is drawn by video.nsh into a single control.
;
; The caption is part of the picture too, which is why the window has no title
; bar: dragging is handled by hit-testing the top strip ourselves.
;
...
Var MediaSize   ; which set of frames and overlays suits this display
Var ArtDir
Var Dialog
Var RunAfter
...
; NSIS puts Back, Next and Cancel back on every page it owns, so this runs on
...
!define MUI_CUSTOMFUNCTION_GUIINIT prismGuiInit

Function prismGuiInit
  ...
  ${EndIf}
  StrCpy $Dpi $0
  ; Pick the set whose frames are the size of the window, so every frame is
  ; drawn one to one. Decoding a 1440 wide JPEG to fill a 960 wide window costs
  ; the frame rate twice over: once to decode, once to rescale.
  ${If} $Dpi >= 180
  ...
  ; A default timer only fires on the 15.6 ms scheduler tick, which turns a 42 ms
  ; request into 47 and the frame rate into a stutter. One millisecond, please.
  ...
  System::Call 'gdi32::CreateFont(i $0, i 0, i 0, i 0, i 400, i 0, i 0, i 0, \
    i 1, i 0, i 0, i 5, i 0, t "Segoe UI") p .r1'
  StrCpy $Font $1
FunctionEnd

; The loop and the overlays ride along inside setup, and the set that matches
; this display is unpacked before the first page draws.
!macro UNPACK_MEDIA
  StrCpy $ArtDir "$PLUGINSDIR\m"
  CreateDirectory "$ArtDir\v"
  CreateDirectory "$ArtDir\o"
  ; The footage is one size for every display: it is defocused ink, and an
  ; upscale is invisible on it. Only the type changes with the DPI.
  SetOutPath "$ArtDir\v"
  File "${BUILD_RESOURCES_DIR}\installer\media\800\v\*.jpg"
  SetOutPath "$ArtDir\o"
  ${If} $MediaSize == "1440"
    File "${BUILD_RESOURCES_DIR}\installer\media\1440\o\*.png"
  ${Else}
    File "${BUILD_RESOURCES_DIR}\installer\media\960\o\*.png"
  ${EndIf}
!macroend
