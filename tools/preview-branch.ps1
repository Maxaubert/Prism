...
$prismPreviewExe = Join-Path $prismWorkspace 'dist\explorer-native-clipboard-trial\win-unpacked\Prism.exe'
$prismPreviewProfile = Join-Path $prismWorkspace '.e2e\explorer-native-clipboard-profile'
if (-not (Test-Path -LiteralPath $prismPreviewExe -PathType Leaf)) {
  throw 'Build the branch first with npm run package -- --config.directories.output=dist/explorer-native-clipboard-trial.'
}
