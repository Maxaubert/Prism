...
$prismPreviewExe = Join-Path $prismWorkspace 'dist\explorer-tab-drop-trial\win-unpacked\Prism.exe'
$prismPreviewProfile = Join-Path $prismWorkspace '.e2e\explorer-tab-drop-profile'
if (-not (Test-Path -LiteralPath $prismPreviewExe -PathType Leaf)) {
  throw 'Build the branch first with npm run package -- --config.directories.output=dist/explorer-tab-drop-trial.'
}
