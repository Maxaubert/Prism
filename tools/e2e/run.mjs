...
/**
 * The phone as a remote (2026-09-07, #107, reworked onto the target control
 * the same day). There is ONE screen on the phone, its own folder explorer,
 * so this scenario walks the way a person does: open a film on the phone,
 * flip the player's target from "This phone" to "This PC", and the film goes
 * over. What is measured here and nowhere else is the HANDOVER: the PC is
 * showing a picture and has no `<video>` at all until the flip, which is what
 * proves the phone told the PC WHAT to open rather than only pressing play on
 * whatever it happened to have. Pause, seek, volume, mute and next are then
 * driven from the phone's own controls, asserted on the PC's `<video>` itself
 * and asserted back on the phone, which hears every one of them over the
 * state stream. Flipping back mounts the phone's own player again, and the
 * choice survives a reload: a phone left on "This PC" hands the NEXT film
 * over on its own, which the last few lines prove with a different file.
 *
 * The latency is MEASURED in both directions and printed with the pass: the
 * flip to the PC's element playing, a tap on Play to the same, and the phone
 * hearing a state back over SSE.
 *
 * The films are 1.5s of testsrc, so the one step that has to happen WHILE one
 * plays - the pause - is taken at once, and nothing after it reads a clock
 * that is still moving.
 */
async function phoneRemoteScenario(fixtures) {
  console.log("phone: the player's target hands a film to the PC and drives it")
  // A picture, deliberately: the PC has no player at all until the phone
  // hands it one, so the handover cannot be read as a lucky press on
  // something the PC was already showing.
  const { app, win } = await launch(join(fixtures, 'one.png'))
  let page = null
  try {
    const { base, token, status } = await pairPhone(win)
    ...
    const untilPhone = (fn, arg = null, timeout = 5000) => page.waitForFunction(fn, arg, { timeout })
    const pcSelected = (name) =>
      win.waitForFunction(
        (n) =>
          document.querySelector('[role="treeitem"][aria-selected="true"]')?.textContent?.includes(n) ??
          false,
        name,
        { timeout: 15000 }
      )

    // The PC is showing a picture: there is nothing to drive, and the routes
    // say so before any phone has drawn anything.
    ok((await win.locator('video').count()) === 0, 'the PC has no player: it is showing a picture')
    ok((await cmd({ op: 'toggle' })).status === 409, 'a command with nothing playing on the PC is 409')
    ok((await cmd({ op: 'seek', to: -1 })).status === 400, 'a bad command is 400')
    // `open` is the one command carrying a path, so it is walled like every
    // other path a phone names, before anything is forwarded.
    ok(
      (await cmd({ op: 'open', path: 'C:\\Windows\\explorer.exe' })).status === 403,
      "an open outside the phone's root is refused"
    )
    ...
    // A picture carries no target control: there is no PC transport to hand
    // a picture or a page of a document to.
    await page.click('[data-phone-file]:has-text("one.png")')
    await page.waitForSelector('[data-phone-viewer][data-kind="image"]', { timeout: 10000 })
    ok((await page.locator('[data-phone-target]').count()) === 0, 'a picture offers no target')
    await page.click('[aria-label="Back to the folder"]')

    // A film, on the phone: the default, and the phone's own player.
    await page.click('[data-phone-file]:has-text("ep1.mp4")')
    await page.waitForSelector('[data-phone-viewer][data-kind="video"] video', { timeout: 10000 })
    ok(
      (await page.locator('[data-phone-target="phone"][aria-checked="true"]').count()) === 1,
      'a fresh phone plays on the phone'
    )
    ok((await page.locator('video').count()) === 1, "the phone's own player is mounted")

    // The flip: the phone's player goes, the panel arrives, and the PC opens
    // THAT film and plays it.
    const tFlip = performance.now()
    await page.click('[data-phone-target="pc"]')
    ...
    ok(
      (await page.locator('[data-phone-viewer][data-target="pc"]').count()) === 1,
      'the viewer says where it is playing'
    )
    ok((await page.locator('[data-phone-file]').count()) === 0, 'the explorer stays behind the film')
    let opened = true
    await win
      .waitForFunction(
        () =>
          (document
            .querySelector('[role="treeitem"][aria-selected="true"]')
            ?.textContent?.includes('ep1.mp4') ?? false) &&
    ...
    ok(
      opened,
      `the PC opens THAT film and plays it (${(performance.now() - tFlip).toFixed(0)}ms from the flip)`
    )

    // Pause, from the phone's own transport, taken AT ONCE: the fixture runs
    // for 1.5s, and everything after this reads a clock that has stopped.
    let pausedByTap = true
    await page.click('[data-phone-remote] [aria-label="Pause"]', { timeout: 5000 }).catch(() => {
      pausedByTap = false
    })
    await untilPc(() => document.querySelector('video')?.paused === true)
    ok(pausedByTap, "a tap on the phone's Pause pauses the PC")
    const tHeard = performance.now()
    await untilPhone(
      () =>
        document.querySelector('[data-phone-remote]')?.getAttribute('data-remote-playing') === 'false'
    )
    ok(true, `and the phone hears it stop (${(performance.now() - tHeard).toFixed(0)}ms over the stream)`)
    await untilPhone(
    ...
    // Play, timed: the tap to the PC's element moving.
    const tTap = performance.now()
    await page.click('[data-phone-remote] [aria-label="Play"]')
    let tapped = true
    await untilPc(() => !(document.querySelector('video')?.paused ?? true)).catch(() => {
      tapped = false
    })
    ok(tapped, `a tap on the phone's Play plays it again (${(performance.now() - tTap).toFixed(0)}ms)`)
    await page.click('[data-phone-remote] [aria-label="Pause"]')
    await untilPc(() => document.querySelector('video')?.paused === true)

    // Seek, from the phone's scrubber: one send on release, and the PC's
    // clock lands where the finger left it. The film is 1.5s, so the
    // assertion is on the PC's own currentTime rather than a 0:0x readout.
    const wasAt = await pcVideo(() => document.querySelector('video')?.currentTime ?? 0)
    const dur = await pcVideo(() => document.querySelector('video')?.duration ?? 0)
    const bar = await page.locator('[data-phone-remote] input[aria-label="Position"]').boundingBox()
    await page.mouse.click(bar.x + bar.width * 0.4, bar.y + bar.height / 2)
    let seeked = true
    await untilPc(() => (document.querySelector('video')?.currentTime ?? 0) > 0.25).catch(() => {
      seeked = false
    })
    const at = await pcVideo(() => document.querySelector('video')?.currentTime ?? 0)
    ok(
      seeked && at > wasAt + 0.1 && Math.abs(at - dur * 0.4) < 0.3,
      `a tap on the phone's scrubber seeks the PC (${at.toFixed(2)}s of ${dur.toFixed(2)}s)`
    )
    let followed = true
    await untilPhone(
      (want) => {
        const el = document.querySelector('[data-phone-remote] input[aria-label="Position"]')
        return !!el && Math.abs(Number(el.value) - want) < 0.25
      },
      at,
      5000
    ).catch(() => {
    ...
    ok(followed, "the phone's scrubber follows the PC's own clock back")

    // Volume, from the phone's slider: a quarter of a 0..200% track. The
    // phone's readout is asserted against the PC's own volume rather than
    // against the pixel that was tapped.
    const vbar = await page.locator('[data-phone-remote] input[aria-label="Volume"]').boundingBox()
    await page.mouse.click(vbar.x + vbar.width * 0.25, vbar.y + vbar.height / 2)
    let quieter = true
    await untilPc(() => (document.querySelector('video')?.volume ?? 1) < 0.9).catch(() => {
      quieter = false
    })
    const vol = await pcVideo(() => document.querySelector('video')?.volume ?? 1)
    ok(
      quieter && vol > 0.1 && vol < 0.9,
      `the phone's slider sets the PC's volume (${Math.round(vol * 100)}%)`
    )
    let shown = true
    await untilPhone(
      (want) => (document.querySelector('[data-remote-vol]')?.textContent ?? '') === want,
      `${Math.round(vol * 100)}%`,
      5000
    ).catch(() => {
      shown = false
    })
    ok(shown, `and the phone shows what the PC is at (${Math.round(vol * 100)}%)`)
    await page.click('[data-phone-remote] [aria-label="Mute"]')
    await untilPc(() => document.querySelector('video')?.muted === true)
    ok(true, 'a tap on Mute mutes the PC')
    await untilPhone(() => (document.querySelector('[data-remote-vol]')?.textContent ?? '') === '0%')
    ok(true, 'and the phone shows 0%')
    await page.click('[data-phone-remote] [aria-label="Unmute"]')
    await untilPc(() => document.querySelector('video')?.muted === false)

    // Next: the PC steps to ep2.mp4 while the phone keeps ep1 on screen (the
    // panel is the PC's state, not the phone's file). EVERY transport verb
    // here is scoped to `[data-phone-remote]`, and that is not tidiness: the
    // viewer's own header carries Next and Previous for paging the FOLDER, and
    // `page.click` is not strict, so an unscoped selector clicks the header's
    // and steps the phone instead - which passed as "Next" while proving
    // nothing about the PC.
    await page.click('[data-phone-remote] [aria-label="Next"]')
    await pcSelected('ep2.mp4')
    await untilPhone(
    ...
    ok(
      (await page.locator('[data-phone-remote] [aria-label="Next"][disabled]').count()) === 1,
      'Next greys out at the last video'
    )
    ok(
      (await page.locator('[data-phone-remote] [aria-label="Previous"]:not([disabled])').count()) === 1,
      'Previous is offered'
    )

    // Back to the phone: the panel goes and the phone's own player returns,
    // for the film the phone still has on screen.
    await page.click('[data-phone-target="phone"]')
    await page.waitForSelector('[data-phone-viewer][data-kind="video"] video', { timeout: 15000 })
    ok((await page.locator('[data-phone-remote]').count()) === 0, 'the remote panel goes')
    ok(
      (await page.locator('[data-phone-viewer][data-target="phone"]').count()) === 1,
      'and the viewer says the phone has it back'
    )
    let back = true
    await untilPhone(() => (document.querySelector('video')?.readyState ?? 0) >= 1, null, 15000).catch(
      () => {
        back = false
      }
    )
    ok(back, "the phone's own player has metadata again")

    // Flipping mid-film hands it over in that direction, both ways: the PC
    // is on ep2 after Next, and the phone is still holding ep1.
    await page.click('[data-phone-target="pc"]')
    await page.waitForSelector('[data-phone-remote]', { timeout: 10000 })
    let handedBack = true
    await pcSelected('ep1.mp4').catch(() => {
      handedBack = false
    })
    ok(handedBack, 'flipping back to the PC hands the film on the phone over again')
    ok(
      (await page.evaluate(() => localStorage.getItem('prism.phone.target'))) === 'pc',
      'the choice is written down'
    )

    // And it survives a reload: a DIFFERENT film, opened from the explorer,
    // goes straight to the PC without being asked twice.
    await page.reload()
    await page.waitForSelector('[data-phone-file]', { timeout: 10000 })
    ok(
      (await page.locator('[data-phone-remote]').count()) === 0,
      'a reload comes back to the explorer, which is the only screen there is'
    )
    await page.click('[data-phone-file]:has-text("ep2.mp4")')
    await page.waitForSelector('[data-phone-remote]', { timeout: 15000 })
    ok(
      (await page.locator('[data-phone-target="pc"][aria-checked="true"]').count()) === 1 &&
        (await page.locator('video, audio').count()) === 0,
      'the remembered choice opens the next film on the PC'
    )
    let handed = true
    await pcSelected('ep2.mp4').catch(() => {
      handed = false
    })
    ok(handed, 'and hands THAT film over on its own')
  } finally {
    await page?.close().catch(() => {})
    await win.evaluate(() => window.prism.phoneSetOn(false, null)).catch(() => {})
    await app.close()
