...
- **One icon per kind** (2026-08-30, #74): the six ProgIDs used to point `DefaultIcon` at
  ...
  Two things were MEASURED rather than assumed. The glyphs carry NO tile: the near-black
  rounded square cost about a fifth of every edge, and 16px (details view) is the size
  almost everyone sees. And they are INDIGO DUOTONE, not white: white glyphs only read
  because the tile was behind them, and bare white on Explorer light mode is invisible -
  `tools/icons/compare.py` puts every treatment on both grounds and shows it. Indigo
  #5b5bd6 as the mass reads on white and on #202020 alike. Every size is DRAWN at that
  size, never downsampled from one big render, and the glyphs are laid out in sixteenths
  of the tile so a 16px frame lands on whole pixels.
